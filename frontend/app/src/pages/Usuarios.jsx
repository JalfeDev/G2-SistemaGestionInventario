...
import { authService, getApiError, rolService, usuarioService } from '../services/api'
...
const selfProtectionMessage = 'No puedes modificar tu propio rol para evitar perder el acceso administrativo.'

export default function Usuarios() {
  const users = useApiResource(usuarioService.listar)
  const roles = useApiResource(rolService.listar)
  const currentUser = useApiResource(authService.me, emptyCurrentUser)
  ...
  async function changeRole(user, rolId) {
    if (!currentUser.data.id) return setStatus({ tone: 'danger', text: 'No se pudo verificar el usuario autenticado.' })
    if (user.id === currentUser.data.id) return setStatus({ tone: 'warning', text: selfProtectionMessage })
  ...
  return (
    <>
      ...
      <ResourceNotice error={users.error || roles.error || currentUser.error} />{status.text && <Notice tone={status.tone}>{status.text}</Notice>}
      <Notice tone="warning">{selfProtectionMessage}</Notice>
      ...
      <Card>{users.loading ? <Loader /> : <div className="table-wrap"><table><thead><tr><th>Colaborador</th><th>Usuario</th><th>Correo</th><th>Rol</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{users.data.map((item) => { const isCurrentUser = item.id === currentUser.data.id; const disabled = !currentUser.data.id || isCurrentUser; return <tr key={item.id}><td><strong>{[item.nombres, item.apellidos].filter(Boolean).join(' ') || '-'}</strong></td><td>{item.usuario}</td><td>{item.email || '-'}</td><td><select disabled={disabled} title={isCurrentUser ? selfProtectionMessage : ''} value={validRoles.find((role) => normalizeRole(role.nombre) === normalizeRole(item.rol))?.id || ''} onChange={(event) => changeRole(item, event.target.value)}>{validRoles.map((role) => <option value={role.id} key={role.id}>{roleLabel(role.nombre)}</option>)}</select></td><td><Badge tone={item.activo ? 'success' : 'danger'}>{item.activo ? 'Activo' : 'Desactivado'}</Badge></td><td><button className="button subtle" disabled={disabled} title={isCurrentUser ? selfProtectionMessage : ''} onClick={() => toggleActive(item)}>{item.activo ? 'Desactivar' : 'Activar'}</button></td></tr> })}</tbody></table></div>}</Card>
    </>
  )
}
